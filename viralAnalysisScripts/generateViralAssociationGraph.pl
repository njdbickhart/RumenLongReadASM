...
use strict;
my $usage = "perl $0 <input blobtools data> <read map association data> <output filename>\n";
...
while(my $line =<$IN>){
	chomp $line;
	my @segs = split(/\t/, $line);
	# Because viral contigs are the last segment, let's associate non-viral as a secondary association in the hash
